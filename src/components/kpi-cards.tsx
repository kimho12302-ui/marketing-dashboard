...
import {
  ...
  BarChart3, ShoppingCart, Target, CreditCard, HelpCircle, ChevronDown, ChevronUp,
} from "lucide-react";
...
function KPICard({ card, data, selectedCard, onCardClick, tooltip, setTooltip }: any) {
  ...
  return (
    <Card className={`${profitBorder} ${clickable} ${isSelected ? "border-indigo-500 ring-1 ring-indigo-500/30" : ""} relative`}
      ...
      <CardContent>
        <div className={`${card.primary ? "text-xl" : "text-lg"} font-bold tracking-tight text-gray-900 dark:text-zinc-100`}>
          {card.prefix || ""}
          {card.isRatio ? card.value.toFixed(2) : formatCompact(card.value)}
          {card.suffix || ""}
          {card.subtitle && (
            <span className="text-xs font-normal text-gray-400 dark:text-zinc-500 ml-1">({card.subtitle})</span>
          )}
        </div>
        {card.prev !== 0 && (
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {primaryCards.map((card) => (
          <KPICard key={card.key} card={card} data={data} selectedCard={selectedCard} onCardClick={onCardClick} tooltip={tooltip} setTooltip={setTooltip} />
        ))}
      </div>
      {/* Secondary toggle */}
      <button
        onClick={() => setShowSecondary(!showSecondary)}
        className="flex items-center gap-1 text-xs text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-zinc-300 transition-colors mx-auto"
      >
        {showSecondary ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
        {showSecondary ? "접기" : "주문수 · AOV · CAC"}
      </button>
      {showSecondary && (
