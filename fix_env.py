...
print(f"Found {len(data.get('envs', []))} env vars")
for env in data.get('envs', []):
    print(f"  {env['key']} (id={env['id']}, value_len={len(env.get('value',''))})")
    # Delete it
    dr = subprocess.run([
        "curl", "-s", "-X", "DELETE",
    ...
    print(f"    deleted: {dr.stdout[:100]}")

# Now add clean env vars
envs = {
...
for key, value in envs.items():
    ...
    if 'created' in resp:
        print(f"  ✅ {key} added (no newlines)")
    else:
        print(f"  ❌ {key}: {r.stdout[:200]}")

# Now trigger deploy via API
...
resp = json.loads(r.stdout)
if 'id' in resp:
    print(f"✅ Deploy started! ID: {resp['id']}")
    print(f"   URL: {resp.get('url', 'N/A')}")
else:
    print(f"❌ Deploy failed: {r.stdout[:300]}")
