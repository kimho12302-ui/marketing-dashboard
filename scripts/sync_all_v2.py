...
SHEET_BALANCELAB = "1FzxDCyR9FyAIduf7Q0lfUIOzvSqVlod21eOFqaPrXio"
...
def main():
    ...
    def sync_ga4_funnel():
        ...
        dedup_upsert(sb, "daily_funnel", rows, "date,brand,channel")
    ...
    # 7. 밸런스랩 공구 매출
    print("\n📊 7. 밸런스랩 공구 매출...")
    def sync_balancelab_gonggu():
        from datetime import datetime
        sheet = gc.open_by_key(SHEET_BALANCELAB)
        ws = sheet.worksheet("밸런스랩 매출 시트")
        rows_data = ws.get_all_values()
        # 헤더: row[0]=DATE, row[10]=공구 매출, row[11]=구매건수, row[12]=구매자
        result = []
        for r in rows_data[4:]:  # 데이터 시작 행
            date_raw = r[0] if r else ""
            if not date_raw.strip(): continue
            # "4월 7일 (화)" → "2026-04-07"
            try:
                ...
                continue
            revenue = safe_num(r[10]) if len(r) > 10 else 0
            quantity = safe_int(r[11]) if len(r) > 11 else 0
            buyers = safe_int(r[12]) if len(r) > 12 else 0
            if revenue <= 0: continue
            avg_price = int(revenue / buyers) if buyers > 0 else int(revenue)
            result.append({
                "date": d, "brand": "balancelab", "channel": "공구",
                "category": "밸런스랩", "product": "공구 합계", "lineup": "",
                "revenue": int(revenue), "quantity": quantity,
                "buyers": buyers, "avg_price": avg_price,
            })
        if not result:
            print("  ⏭ 데이터 없음")
            return
        # 기존 공구 데이터 삭제 후 재삽입 (unique constraint 없음)
        dates_to_sync = list(set(r["date"] for r in result))
        for d in dates_to_sync:
            sb.table("product_sales").delete().eq("date", d).eq("brand", "balancelab").like("channel", "공구%").execute()
        for i in range(0, len(result), 200):
            sb.table("product_sales").insert(result[i:i+200]).execute()
        print(f"  ✅ product_sales 공구: {len(result)}건 완료")

    try:
        retry_with_backoff(sync_balancelab_gonggu, "밸런스랩 공구")
    except Exception as e:
        errors.append(str(e))
        print(f"  ❌ {e}")
